...
const Popup = ({ delType, todoMass = "", id, className, }) => {
    const { deleteTodo, addPopup } = useTodo()
            ...
            <div className={`${className}  bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-200 dark:border-gray-700`}>
                <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{delType}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400 break-words">{
                            `Are you sure you want to delete "${todoMass}"? This action cannot be undone.`
                        }</p>
                    </div>
                </div>
                <div className="flex gap-3 justify-end text-black dark:text-white">
                    <button
                        onClick={() => addPopup(id)}
